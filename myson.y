...
;
expression: T_INT				
	  | expression T_PLUS expression	{ $$ = bignum_to_string(add_bignum(string_to_bignum($1),string_to_bignum($3)));}
	  | expression T_MINUS expression	{ $$ = bignum_to_string(subtract_bignum(string_to_bignum($1),string_to_bignum($3)));}
	  | expression T_MULTIPLY expression	{ $$ = bignum_to_string(product(string_to_bignum($1),string_to_bignum($3)));;}
	  | expression T_DIVIDE expression	{ $$ = bignum_to_string(divide(string_to_bignum($1),string_to_bignum($3)));}
	  | T_LEFT expression T_RIGHT		{ $$ = $2;}
	  | T_SQRT expression				{ $$ = bignum_to_string(square_root(string_to_bignum($2)));}
	  | T_LOG  expression   			{ $$ = bignum_to_string(logarithm(string_to_bignum($2)));}
	  | T_POW T_LEFT expression T_COMMA expression T_RIGHT  { $$ = bignum_to_string(power(string_to_bignum($3),string_to_bignum($5)));}
